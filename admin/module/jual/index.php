...
<?php
        ...
        // Proteksi: Owner tidak bisa akses halaman Transaksi Jual
        if(isset($hasil['role']) && $hasil['role'] === 'owner') {
            echo "<script>alert('Akses ditolak! Anda tidak memiliki izin untuk halaman ini.'); window.location.href='index.php?page=laporan';</script>";
            exit;
        }
...
$(document).ready(function(){
	...
	function formatRupiah(angka) {
		var isNegative = parseFloat(angka) < 0;
		var absAngka = Math.abs(angka).toString().replace(/[^,\d]/g, '');
		var split = absAngka.split(',');
		...
		return isNegative ? '- Rp ' + (rupiah ? rupiah : '0') : 'Rp ' + (rupiah ? rupiah : '0');
	}
...
// Setup bayar field - using event delegation, only needs to be setup once
	...
	function hitungKembali() {
		...
		var kembaliDisplay = formatRupiah(kembali.toFixed(0));
		var kembaliField = $("#kembali");
		
		kembaliField.val(kembaliDisplay);
		
		// Ubah warna dan styling jika uang kurang
		if(kembali < 0) {
			kembaliField.css('color', '#dc3545');
			kembaliField.css('background-color', '#ffe6e6');
		} else {
			kembaliField.css('color', '#007bff');
			kembaliField.css('background-color', '');
		}
	}
