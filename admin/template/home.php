<?php 
// Cek role - jika owner redirect ke laporan
if(isset($_SESSION['admin']['role'])){
    $current_role = trim($_SESSION['admin']['role']);
...
<br/>
<?php 
    $min_stok = isset($toko['min_stok']) && $toko['min_stok'] > 0 ? (int)$toko['min_stok'] : 3;
    $sql="SELECT * FROM barang WHERE CAST(stok AS UNSIGNED) <= ?";
    $row = $config->prepare($sql);
    $row->execute([$min_stok]);
    $r = $row->rowCount();
    if($r > 0){
        echo "
        <div class='alert alert-warning'>
            <span class='glyphicon glyphicon-info-sign'></span> 
            Ada <span style='color:red'>$r</span> barang yang Stok tersisa sudah kurang dari $min_stok items. silahkan pesan lagi !!
