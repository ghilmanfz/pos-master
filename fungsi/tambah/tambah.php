...
if (!empty($_SESSION['admin'])) {
    ...
    if ($userAction !== '') {
        ...
        if (!in_array($role, $allowedRoles, true)) {
            error_log("WARNING: Invalid role '{$role}' in tambah user. Setting to 'kasir'");
        ...
        // DEBUG Log
        error_log("=== DEBUG TAMBAH USER ===");
        error_log("Nama: " . $nama);
        error_log("Role: '" . $role . "'");
        error_log("Role Length: " . strlen($role));
        error_log("Email: " . $email);
        error_log("User: " . $user);
        error_log("Pass length: " . strlen($pass));

        if ($nama === '' || $role === '' || $user === '' || $pass === '') {
            error_log("ERROR: Form tidak lengkap - nama=" . ($nama === '' ? 'empty' : 'ok') . 
                     ", role=" . ($role === '' ? 'empty' : 'ok') . 
                     ", user=" . ($user === '' ? 'empty' : 'ok') . 
                     ", pass=" . ($pass === '' ? 'empty' : 'ok'));
