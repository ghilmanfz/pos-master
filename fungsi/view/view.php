...
class view
{
    ...
    {
        // Get min_stok from toko settings
        $sqlToko = "SELECT min_stok FROM toko WHERE id_toko='1' LIMIT 1";
        $rowToko = $this->db->prepare($sqlToko);
        $rowToko->execute();
        $toko = $rowToko->fetch();
        $minStok = isset($toko['min_stok']) ? (int)$toko['min_stok'] : 3;
        
        $sql = "select barang.*, kategori.id_kategori, kategori.nama_kategori
                from barang inner join kategori on barang.id_kategori = kategori.id_kategori 
                where CAST(stok AS UNSIGNED) <= ? 
                ORDER BY id DESC";
        $row = $this-> db -> prepare($sql);
        $row -> execute(array($minStok));
        $hasil = $row -> fetchAll();
        return $hasil;
    }
    ...
    public function day_of_week_jual($dayOfWeek)
    {
        // Convert dari form numbering (1=Senin/Monday) ke MySQL DAYOFWEEK (1=Minggu/Sunday, 2=Senin/Monday)
